...
public class MainActivity extends AppCompatActivity implements ForecastFragment.Callback, GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {
    ...
    public void onConnected(Bundle bundle) {
        ...
        Uri weather_loc = WeatherContract.WeatherEntry.buildWeatherLocationWithDate(loc, System.currentTimeMillis());
        Cursor cursor = new CursorLoader(this, weather_loc, new String[] {
        ...
        }, null, null, WeatherContract.WeatherEntry.COLUMN_DATE+" ASC").loadInBackground();

        if(cursor != null) {
            cursor.moveToPosition(0);
            PutDataMapRequest dataMapRequest = PutDataMapRequest.create("/weather");
            DataMap map = dataMapRequest.getDataMap();
            map.putString("min_temp", Utility.formatTemperature(this, cursor.getDouble(DetailFragment.COL_WEATHER_MIN_TEMP)));
            map.putString("max_temp", Utility.formatTemperature(this, cursor.getDouble(DetailFragment.COL_WEATHER_MAX_TEMP)));
            map.putInt("weather_id", cursor.getInt(DetailFragment.COL_WEATHER_CONDITION_ID));
